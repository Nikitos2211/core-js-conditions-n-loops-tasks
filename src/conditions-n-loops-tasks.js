...
function getSpiralMatrix(size) {
  ...
  while (layer < Math.ceil(size / 2)) {
    for (let i = layer; i < size - layer; i += 1) {
      matrix[layer][i] = currentNumber;
      currentNumber += 1;
    }

    for (let i = layer + 1; i < size - layer; i += 1) {
      matrix[i][size - layer - 1] = currentNumber;
      currentNumber += 1;
    }

    for (let i = size - layer - 2; i >= layer; i -= 1) {
      matrix[size - layer - 1][i] = currentNumber;
      currentNumber += 1;
    }
...
function getNearestBigger(number) {
  ...
  digits[pivot] = digits[smallestLargerIndex];
